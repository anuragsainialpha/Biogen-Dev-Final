...
router.put('/swap', [auth_1.default], other_1.asyncRouteHandler(async (req, res) => {
    ...
    partys[0] = await party_1.Party(domain, instance).findByIdAndUpdate({ _id: partys[0]._id }, partys[0], { new: true }).exec();
    partys[1] = await party_1.Party(domain, instance).findByIdAndUpdate({ _id: partys[1]._id }, partys[1], { new: true }).exec();
    let x = await party_1.Party(domain, instance).find({ name: partys[1].name });
    let y = await party_1.Party(domain, instance).find({ name: partys[0].name });
    if (x.length > 1) {
        await party_1.Party(domain, instance).findByIdAndDelete({ _id: x[1]._id }).exec();
    }
    if (y.length > 1) {
        await party_1.Party(domain, instance).findByIdAndDelete({ _id: y[1]._id }).exec();
    }
    return res.status(200).json({ data: partys });
}));
async function getPartyDisplayDataFromDb(domain, instance, display) {
    return await party_1.Party(domain, instance)
        .find({
        $or: [
            {
                display: display,
