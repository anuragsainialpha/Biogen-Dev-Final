...
router.put('/swap', [auth_1.default], other_1.asyncRouteHandler(async (req, res) => {
    ...
    trxs[0] = await transaction_1.Transaction(domain, instance).findByIdAndUpdate({ _id: trxs[0]._id }, trxs[0], { new: true }).exec();
    trxs[1] = await transaction_1.Transaction(domain, instance).findByIdAndUpdate({ _id: trxs[1]._id }, trxs[1], { new: true }).exec();
    let x = await transaction_1.Transaction(domain, instance).find({ name: trxs[1].name });
    let y = await transaction_1.Transaction(domain, instance).find({ name: trxs[0].name });
    if (x.length > 1) {
        await transaction_1.Transaction(domain, instance).findByIdAndDelete({ _id: x[1]._id }).exec();
    }
    if (y.length > 1) {
        await transaction_1.Transaction(domain, instance).findByIdAndDelete({ _id: y[1]._id }).exec();
    }
    return res.status(200).json({ data: trxs });
}));
async function getTrxDisplayDataFromDb(domain, instance, display) {
    return await transaction_1.Transaction(domain, instance)
        .find({
        $or: [
            {
                display: display,
