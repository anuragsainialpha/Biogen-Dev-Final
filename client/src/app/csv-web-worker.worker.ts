...
addEventListener('message', async (msg) => {
  ...
  console.log('worker input',msg.data);
  ...
  console.log('csvDataToObject:',result);
  postMessage({
    result: result.data,
    error: result.error,
...
function  sortTransmissionAndRemoveNumber(data1: any){
  data1.sort((a,b) => a-b);
  console.log(data1);
  data1.forEach((rec,recIndex) => {
    //console.log('sortTransmission','recIndex:',recIndex,'value:',rec[recIndex]);
...
function getLinesMerged(
  ...
  objectPaths = sortTransmissionAndRemoveNumber(objectPaths);
...
function getObjectPaths(
  ...
  for (let rowIndex = 0; rowIndex < csvRow.length; rowIndex++) {
    ...
    for (let csvHeaderCounter = 0; csvHeaderCounter < csvHeader.length; csvHeaderCounter++) {
      ...
      //console.log(csvHeaderCounter,row,cellValue);
...
function getValidatedObjPathValue(
  ...
  //console.log('getValidatedObjPathValue',csvHeaderPaths[csvHeaderCounter]);
}
...
function getObjAttributePropsFields(
  ...
  //console.log('getObjAttributePropsFields',csvHeaderPaths[csvHeaderCounter]);
  const propPaths = csvHeaderPaths[csvHeaderCounter];
  propPaths.forEach((propPath, indice) => {
    //if (indice === 0) { -- changing for empty value--XML Fix--Anurag Saini
    //console.log('pathCollector[propPath.path]',pathCollector[propPath.path]);
    if (propPath.value === '') {
...
function getDefaultAndMandatoryFields(
  ...
  //console.log('CSV Header:',csvHeaderPaths[csvHeaderCounter]);//XML Fix--Anurag Saini

  if (autoField && autoField.defaultValue) {
    //console.log('First if',autoField, autoField.defaultValue, csvHeaderPaths[csvHeaderCounter]);
    pathCollector[csvHeaderPaths[csvHeaderCounter]] = autoField.defaultValue;
  } else if (autoField.disabled) {
    //console.log('Else if',autoField, autoField.defaultValue, csvHeaderPaths[csvHeaderCounter]);
    if (autoField.disabled && autoGenDisplayFields.includes(autoField.name)) {
      ...
      //console.log('default value:',defaultGenValue);
      //console.log('Path Collector:',csvHeaderPaths[csvHeaderCounter],pathCollector[csvHeaderPaths[csvHeaderCounter]]);
      const propPaths = csvHeaderPaths[csvHeaderCounter];
      //console.log('propPaths:',propPaths);
      propPaths.forEach((propPath, indice) => {
        //if (indice === 0) { -- changing for empty value--XML Fix--Anurag Saini
        //console.log('pathCollector[propPath.path]:',propPath.path);
        if (propPath.value === '') {
          //console.log('pathCollector[propPath.path]:',propPath.path);
          pathCollector[propPath.path] = defaultGenValue;
          //console.log(pathCollector[propPath.path] );
        }}
      )
      //console.log(pathCollector[csvHeaderPaths[csvHeaderCounter]]);
    } else response.error.push(`${autoField.displayText} is missing at row  ${rowIndex}`);
  }
  //console.log('getDefaultAndMandatoryFields-autoField',pathCollector[csvHeaderPaths[csvHeaderCounter]]);
