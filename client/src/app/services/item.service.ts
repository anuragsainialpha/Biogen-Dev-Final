...
export class ItemService implements WebWorkerInputService {
  ...
  getViewUrl: ViewUrl = (viewURL, csvRowIndex, statusInitial) => {
    ...
    let itemIDIndex = this.csvHeader.findIndex(
      (d) => d === this.dragdropListTc[this.itemIdIndex+1].displayText.toUpperCase()//Added Index
    );
    //let itemDomainIndex = this.csvHeader.findIndex((d) => d === this.dragdropListTc[this.itemIdDomain].displayText); --XML Fix--Anurag Saini
    let itemDomainIndex = this.csvHeader.findIndex((d) => d === 'Domain Name');
    if (itemIDIndex === -1) {
      itemIDIndex = this.csvHeaderAll[csvRowIndex].findIndex(
        (d) => d === this.dragdropListTc[this.itemIdIndex+1].path[0] //--XML Fix--Anurag Saini
      );
      // itemDomainIndex = this.csvHeaderAll[csvRowIndex].findIndex(--XML Fix--Anurag Saini
      //   (d) => d === this.dragdropListTc[this.itemIdDomain].path[0]--XML Fix--Anurag Saini
