...
export class TransmissionService {
  ...
  public getJSONKeyValueXML = (data) => {
   let xmlvalue: any[];
   let xmlheader = '<Transmission><TransmissionBody><GLogXMLElement><gtm:GtmContact>';
   let xmltail = '</gtm:GtmContact></GLogXMLElement></TransmissionBody></Transmission>';
   let contacttagStart = '<gtm:Contact>';
   let contacttagEnd = '</gtm:Contact><gtm:TransactionCode>IU</gtm:TransactionCode>';
   let CompanyNametag: string = '';
   let contactgidxml: string = '';
   let LocationReftaghead = '<gtm:LocationRef><otm:Location><otm:TransactionCode>IU</otm:TransactionCode>';
   let LocationReftagtail = '</otm:Location></gtm:LocationRef>';
   let Addresstaghead = '<otm:Address>';
   let Addresstagtail = '</otm:Address>'; 
   let AddressLinetag1 = '';
   let AddressLinetag2 = '';
   let AddressLinetag3 = '';
   let locationgidtag = '';
   ...
   for (var key in data[0]) {
    ...
    if(key ==='Transmission.TransmissionBody.GLogXMLElement.gtm:GtmContact.gtm:Contact.otm:ContactGid.otm:Gid.otm:Xid')
    {
      contactgidxml = '<otm:ContactGid><otm:Gid><otm:DomainName>BIIB</otm:DomainName><otm:Xid>'+data[0][key]+'</otm:Xid></otm:Gid></otm:ContactGid>';
      locationgidtag = '<otm:LocationGid><otm:Gid><otm:DomainName>BIIB</otm:DomainName><otm:Xid>GTMPARTY-'+data[0][key]+
      '</otm:Xid></otm:Gid></otm:LocationGid>'
      }

    if(key ==='Transmission.TransmissionBody.GLogXMLElement.gtm:GtmContact.gtm:Contact.otm:CompanyName')
    {
      CompanyNametag = '<otm:CompanyName>'+ data[0][key].replace('&','&amp;') +'</otm:CompanyName>';
    }
    
    if(key ==='Transmission.TransmissionBody.GLogXMLElement.gtm:GtmContact.gtm:LocationRef.otm:Location.otm:Address.otm:AddressLines.0.otm:AddressLine'){
      AddressLinetag1 = '<otm:AddressLines><otm:SequenceNumber>1</otm:SequenceNumber><otm:AddressLine>'+data[0][key].replace('&','&amp;')+'</otm:AddressLine></otm:AddressLines>';
    }
    if(key ==='Transmission.TransmissionBody.GLogXMLElement.gtm:GtmContact.gtm:LocationRef.otm:Location.otm:Address.otm:AddressLines.1.otm:AddressLine'){
      AddressLinetag2 = '<otm:AddressLines><otm:SequenceNumber>1</otm:SequenceNumber><otm:AddressLine>'+data[0][key].replace('&','&amp;')+'</otm:AddressLine></otm:AddressLines>';
    }
    
    if(key ==='Transmission.TransmissionBody.GLogXMLElement.gtm:GtmContact.gtm:LocationRef.otm:Location.otm:Address.otm:AddressLines.2.otm:AddressLine'){
      AddressLinetag3 = '<otm:AddressLines><otm:SequenceNumber>1</otm:SequenceNumber><otm:AddressLine>'+data[0][key].replace('&','&amp;')+'</otm:AddressLine></otm:AddressLines>';
    }
    if(key ==='Transmission.TransmissionBody.GLogXMLElement.gtm:GtmContact.gtm:LocationRef.otm:Location.otm:Address.otm:City'){
      citytag = '<otm:City>'+data[0][key].replace('&','&amp;')+'</otm:City>';
    }
    if(key ==='Transmission.TransmissionBody.GLogXMLElement.gtm:GtmContact.gtm:LocationRef.otm:Location.otm:Address.otm:PostalCode'){
      postalcodetag = '<otm:PostalCode>'+data[0][key]+'</otm:PostalCode>';
    }
    if(key ==='Transmission.TransmissionBody.GLogXMLElement.gtm:GtmContact.gtm:LocationRef.otm:Location.otm:Address.otm:CountryCode3Gid.otm:Gid.otm:Xid'){
      countrycode3gidtag = '<otm:CountryCode3Gid><otm:Gid><otm:Xid>'+data[0][key].replace('&','&amp;')+'</otm:Xid></otm:Gid></otm:CountryCode3Gid>';
    }
    if(key ==='Transmission.TransmissionBody.GLogXMLElement.gtm:GtmContact.gtm:LocationRef.otm:Location.otm:Address.otm:Province'){
      provincetag = '<otm:Province>'+data[0][key].replace('&','&amp;')+'</otm:Province>';
    }
  
//    else { console.log('failed key comparision',key);}
}
   //xmlvalue[0] = data['a10101.Transmission.TransmissionBody.GLogXMLElement.gtm:GtmContact.gtm:Contact.otm:ContactGid.otm:Gid.otm:DomainName'].value;
   //console.log(jsonData['a10101.Transmission.TransmissionBody.GLogXMLElement.gtm:GtmContact.gtm:Contact.otm:ContactGid.otm:Gid.otm:DomainName']); 
  if (AddressLinetag1 !='' || AddressLinetag2 != '' || AddressLinetag3 != '' || citytag != '' || provincetag == '' || countrycode3gidtag != '' || postalcodetag != '') {
    Addressfull = Addresstaghead + AddressLinetag1 + AddressLinetag2 + AddressLinetag3 + citytag + provincetag + postalcodetag + countrycode3gidtag + Addresstagtail
  }
   return xmlheader + contacttagStart + contactgidxml + CompanyNametag + contacttagEnd + LocationReftaghead + locationgidtag + Addressfull + LocationReftagtail + xmltail;
  }
  ...
  private triggerWebWorker(workerData: WebWorkerInput, itemService: WebWorkerInputService) {
    ...
    worker.onmessage = ({ data }) => {
      ...
      if (this.instanceOfService(itemService, 'PartyService')) {
        ...
        this.getXMLdataManuallyFormulated(data);
        this.getXMlsFromWebWorker(itemService, data);

 //       this.sortTransmissionAndRemoveNumber(data);
      ...
      //console.log('After processing webworker',data);
    };
  }

  private getXMLdataManuallyFormulated(data: any[]){
  console.log('calling individually');    

  }

  private  sortTransmissionAndRemoveNumber(data: any){
    data.result.sort((a,b) => a-b);
    data.result.forEach((rec,recIndex) => {
      console.log(rec);
      rec = String(rec).slice(7);
    });
  }
  ...
  public async sendDataToGtm(itemService: WebWorkerInputService, gidMatchString: string, considerXid: boolean = false) {
    setTimeout(async () => {
      ...
      let progressCount = 0;
      let xids = [];
      //console.log(itemService.xmlArray);
      
      //console.log('itemservicearraylength:',itemService.xmlArray.length);
      while (minTransmission <= itemService.xmlArray.length) {
        ...
        //console.log('chunked Transmissions',chunckedTransmissions);
        if (this.instanceOfService (itemService, 'PartyService')){
          chunckedTransmissions[0] = (await this.getJSONKeyValueXML(chunckedTransmissions)) as unknown;
        ...
        console.log('XML Output',chunckedTransmissions);
...
console.log(gidMatchString,xidContentString);
  ...
  private getRequestBody(chunckedTransmissions: any[], itemService: WebWorkerInputService) {
    ...
    //console.log('Final XML before cleanup',finalXML);
    
    const reqBody = { xml: finalXML, xmlNs: itemService.dragdropList[0].xmlNs };
   // console.log('Final XML after cleanup:',reqBody    );
  ...
  private getFinalXMl(chunckedTransmissions: any[]) {
    ...
    //xml validation for item
    transBody = transBody.replace('<otm:TransactionCode>IU</otm:TransactionCode></otm:Item>','</otm:Item>');
   // transBody = transBody.replace('<otm:DomainName>BIIB</otm:DomainName></otm:Gid>','</otm:Gid>');
   transBody = transBody.replace('<otm:ItemMaster><otm:Item><otm:ItemGid><otm:Gid>',
                '<otm:ItemMaster><otm:Item><otm:TransactionCode>IU</otm:TransactionCode><otm:ItemGid><otm:Gid>');
    // transBody = transBody.replace('<otm:ItemMaster><otm:Item><otm:ItemGid><otm:Gid>',
    //             '<otm:ItemMaster><otm:Item><otm:TransactionCode>IU</otm:TransactionCode><otm:ItemGid><otm:Gid><otm:DomainName>BIIB</otm:DomainName>');
    // transBody = transBody.replace('<otm:TransactionCode>IU</otm:TransactionCode><otm:GtmProdClassTypeGid><otm:Gid><otm:Xid>ECCN US</otm:Xid></otm:Gid></otm:GtmProdClassTypeGid></otm:GtmItemClassification>','</otm:GtmItemClassification>');
    // transBody = transBody.replace('<otm:ClassificationCode>','<otm:GtmProdClassTypeGid><otm:Gid><otm:Xid>ECCN US</otm:Xid></otm:Gid></otm:GtmProdClassTypeGid><otm:ClassificationCode>');
    
    //for Party
    // transBody = transBody.replace('<gtm:TransactionCode>IU</gtm:TransactionCode></gtm:GtmContact>','</gtm:GtmContact>');
    // transBody = transBody.replace('</gtm:Contact><gtm:LocationRef>','</gtm:Contact><gtm:TransactionCode>IU</gtm:TransactionCode><gtm:LocationRef>');
    // transBody = transBody.replace('<otm:TransactionCode>IU</otm:TransactionCode></otm:Location>','</otm:Location>');
    // transBody = transBody.replace('<otm:Location><otm:LocationGid>','<otm:Location><otm:TransactionCode>IU</otm:TransactionCode><otm:LocationGid>');
  ...
  private getXidIndex(itemService: WebWorkerInputService) {
    return itemService.csvHeader.findIndex(
      (d) => d === itemService.dragdropListTc[itemService.itemIdIndex+1].displayText
    );
  }
