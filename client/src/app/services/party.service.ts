...
export class PartyService implements WebWorkerInputService {
  ...
  getViewUrl: ViewUrl = (viewURL, csvRowIndex, statusInitial) => {
    ...
    let partyIDIndex = this.csvHeader.findIndex(
      (d) => d === this.dragdropListTc[this.itemIdIndex+1].displayText.toUpperCase()
    );
    //let partyDomainIndex = this.csvHeader.findIndex((d) => d === this.dragdropListTc[this.itemIdDomain].displayText);
    let partyDomainIndex = this.csvHeader.findIndex((d) => d === 'Domain Name');//--XML Fix--Anurag Saini
    if (partyIDIndex === -1) {
      ...
      //partyDomainIndex = this.csvHeaderAll[csvRowIndex].findIndex(//--XML Fix--Anurag Saini
